...
var listpost = function (req, res) {
  ...
  console.log('post 모듈 안에 있는 listpost 호출 - 요청 파라미터 : ' + paramPage + ', ' + paramPerPage);
...
var addcomment = function (req, res) {
  ...
  if (database.db) {
    ...
    // kimbal add 2018-10-10
    database.UserModel.findByEmail(paramWriter, function(err, results){
      console.log("check find_result:", results.length)  
      if (err || (results.length == 0)) {
        res.writeHead('200', {'Content-Type': 'text/html;charset=utf8'});
        if (err) {
          console.error('게시판 댓글 추가 중 에러 발생' + paramWriter + ', ' + err.stack);
          res.write('<h2>게시판 댓글 추가 중 에러 발생</h2>');
        } else {
          console.error('게시판 댓글 추가는 로그인 계정이 있어야 합니다. ID:' + paramWriter);
          res.write('<h2>게시판 댓글 추가는 로그인 계정이 있어야 합니다</h2>');
        }
        res.end();
        return;
      }
